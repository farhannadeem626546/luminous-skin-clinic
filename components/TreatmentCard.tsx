...
export default function TreatmentCard({ treatment }: { treatment: Treatment }) {
  ...
  return <article className="treatment-card">
    <div className="card-image">{treatment.videoUrl?<video src={treatment.videoUrl} muted loop playsInline autoPlay preload="metadata"/>:<Image src={src} alt={treatment.name} fill sizes="(max-width: 768px) 100vw, 33vw" />}</div>
    <div className="card-body"><span className="card-category">{treatment.category}</span><h3>{treatment.name}</h3><p>{treatment.short}</p>
      ...
      <div className="card-actions"><Link href={`/treatments/${treatment.slug}`} className="text-link">Learn more →</Link><Link href={`/booking?treatment=${treatment.slug}`} className="card-book-button">Book now</Link></div>
    </div>
  </article>;
}
