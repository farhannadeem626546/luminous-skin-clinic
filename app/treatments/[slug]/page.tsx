...
export default async function TreatmentDetail({params}:{params:Promise<{slug:string}>}){
  ...
  return <>
    <section className="detail-hero"><div className="container detail-grid"><div className="detail-copy"><span className="eyebrow">{t.category}</span><h1>{t.name}</h1><p>{t.short}</p><div className="detail-facts"><div><span>Starting from</span><strong>£{t.price}</strong></div><div><span>Appointment</span><strong>{t.duration} min</strong></div></div><div className="hero-actions"><Link className="button primary" href={`/booking?treatment=${t.slug}`}>Book this treatment</Link><Link className="button secondary" href="/contact">Ask a question</Link></div></div><div className="detail-image">{t.videoUrl?<video src={t.videoUrl} controls playsInline preload="metadata"/>:<Image src={src} alt={t.name} fill priority sizes="(max-width:900px) 100vw, 50vw"/>}</div></div></section>
