...
export default async function Home() {
  const [cms,treatments,dbReviews,trustpilot] = await Promise.all([getCms(),getTreatments(),getReviews(),getTrustpilotSummary()]);
  ...
  return <>
    ...
    <section className="section soft"><div className="container"><div className="title-row"><SectionTitle eyebrow="Popular appointments" title="Skin care made easier to choose" text="Clear treatment information, transparent starting prices and a simple route to booking."/><Link href="/services" className="text-link desktop-link">View all services →</Link></div><div className="treatment-grid">{treatments.slice(0,4).map(t=><TreatmentCard key={t.slug} treatment={t}/>)}</div></div></section>
